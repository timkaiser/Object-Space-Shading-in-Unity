...
public class MyPipelineTextureArray : RenderPipeline {
    //### variables ################################################################################################################
    CullResults cull;
    ...
    //### Constructor ##############################################################################################################
    public MyPipelineTextureArray() {
        //Log("_________________________________________________________________________________________________");
        //compute shader
    ...
    public void Render(ScriptableRenderContext context, Camera camera) {
        ...
        //Log("context.SetupCameraProperties(camera)");
        ...
        foreach (ObjData obj in sceneObjects) {
            ...
            obj.tileMask = runTileMaskShader(obj.id, rts[0], rts[1], obj.DebugMap);
            Log(obj.obj.name + " tileShader");
            obj.worldPosMap = runWorldPosMapShader(obj.tileMask, obj);
            Log(obj.obj.name + " worldPosShader");
    ...
    List<ObjData> uvRenderer(ScriptableRenderContext context, Camera camera) {
        ...
        foreach (MeshFilter mesh in GameObject.FindObjectsOfType<MeshFilter>()) {
            ...
            obj.DebugMap = new RenderTexture(MAX_TEXTURE_SIZE, MAX_TEXTURE_SIZE, 32, RenderTextureFormat.ARGBFloat);
            obj.DebugMap.filterMode = FilterMode.Point;
            obj.DebugMap.anisoLevel = 0;
            obj.DebugMap.enableRandomWrite = true;
            obj.DebugMap.Create();
    ...
    RenderTexture runTileMaskShader(int objID, RenderTexture IDandMip, RenderTexture uv, RenderTexture debugMap) {
...
#if DEBUG
        tileMaskShader.SetTexture(tileMaskKernel, "DebugOut", debugMap);
#endif
    ...
    void Timestamp(string name) {
        //Debug.Log(name+": "+ (Time.realtimeSinceStartup - lastTimestamp));
        lastTimestamp = Time.realtimeSinceStartup;
    }

    //log to file
    float lastTimestampLog = 0;
    int index = 0;
    StringBuilder log;
    void Log(String name) {
        ...
        log.Append(index++).Append(" | ").Append(name).Append(" | ").Append(Time.realtimeSinceStartup).Append(" | ").Append(Time.realtimeSinceStartup - lastTimestampLog).Append(" | ")
            .Append(1 / Time.deltaTime).Append(" | ").Append(stackFrame.GetFileLineNumber()).Append(" | ").Append(stackFrame.GetMethod()).Append("\n");

        lastTimestampLog = Time.realtimeSinceStartup;
    }

    void writeLog() {
        File.AppendAllText(@"c:\Users\TIm\Desktop\log.txt", log.ToString());
        log = new StringBuilder();
    }
